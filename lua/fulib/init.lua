...
M.all = function(pred, tbl)
  ...
  return _15_(pred, tbl)
end
M.any = function(pred, tbl)
  local function _17_(...)
    return not pred(...)
  end
  return not M.all(_17_, tbl)
end
...
M.for_each = function(f, tbl)
  local _18_
  if M["list?"](tbl) then
    _18_ = for_each_in_lst
  else
    _18_ = for_each_in_tbl
  end
  return _18_(f, tbl)
end
...
M.zip_with = function(f, lst1, lst2)
  ...
  return M.map(_34_, range(1, len))
end
M.zip = function(lst1, lst2)
  local function _35_(_241, _242)
    return {_241, _242}
  end
  return M.zip_with(_35_, lst1, lst2)
end
return M
