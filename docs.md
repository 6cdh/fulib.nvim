...
This lib uses haskell-like type annotation. Its syntax is

    first_argument_type [-> second_argument_type [-> ...]] -> return type

For example, a function with type `a -> b` means it accepts a type `a`
parameter and have return a type `b` value. The type annotation `a -> b
-> c` means it accepts two parameters: the first is type `a` and the
second type `b`, and return type `c`. \* `bool`: bool \* `number`:
number \* `string`: string \* `list` or `[v]`: the tables that is empty
or contains not-nil value at the index 1 \* `table` or `{k v}`: table \*
`any`: any type \* `(a -> b)`: function type that accepts type `a` and
return type `b` \* `[a|b]`: type `a` or type `b` \#\# number
...
O(1). Increase `num`. `num` may be a number or string that can be
...
O(1). Decrease `num`. `num` may be a number or string that can be
converted to a number.
...
O(1). Return true if `num` is odd, false otherwise. If `num` is a
string, it must be convertible to a number.
...
O(1). Return true if `num` is even, false otherwise. If `num` is a
string, it must be convertible to a number.
...
O(n). Insert `elem` in front of `lst`. A new list would be created and
returned.
...
O(n). Remove the first element of `lst` and return `lst`. A new table
...
O(n \* pred). Return true if predicate `pred` return true for all
elements of `tbl`, false otherwise.
...
O(n \* pred). Return true if predicate `pred` return true for at least
elements of `tbl`, false otherwise.
...
O(n \* f). Apply function `f` for all elements of `tbl` without change
`tbl` or create a new list.
...
O(n \* pred). Return a new list with the elements of `tbl` for which
`pred` returns true.
...
O(n \* f). Start with `init`, reduce `lst` with function `f`, from left
to right.
...
O(n \* f). Start with `init`, reduce `lst` with function `f`, from right
to left.
